...
// Using the bookCollection array, write a function that:
...
//   ...
// }
// Then print each genre and its total pages, for example:
// "Science Fiction: 898 pages"
// "Fantasy: 972 pages"
// Hint: Work with nested arrays (genre), Use an object as an accumulator (keys as genres, values as total pages).

function createNewObject(pagesPerGenre) {
  let newObject = {};
  for (let index = 0; index < pagesPerGenre.length; index++) {
    for (let j = 0; j < pagesPerGenre[index].genre.length; j++) {
      if (newObject[bookCollection[index].genre[j]]) {
        newObject[pagesPerGenre[index].genre[j]] += bookCollection[index].pages;
      } else {
        newObject[pagesPerGenre[index].genre[j]] = bookCollection[index].pages;
      }
    }
