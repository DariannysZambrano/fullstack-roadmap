...
function getBooksByChaptersNumber(chapters312) {
    let capitulos = [];
  for (let index = 0; index < bookCollection.length; index++){
    if (bookCollection[index].chapters >= chapters123) {
      capitulos.push(bookCollection[index]);
    }
  }return capitulos;
}

console.log(getBooksByChaptersNumber(40));




function getBooksByGenre(texto) {
 let newArray = [];
  for (let index = 0; index < bookCollection; index++) {
    if(bookCollection[index].genre === texto){
...
function getBooksByAmazonRating(num) {
  let newArray = [];
  for (let index = 0; index < bookCollection.length; index++) {
    if (bookCollection[index].score >= num) {
      newArray.push(bookCollection[index]);
    }
  } return newArray;
}

console.log(getBooksByAmazonRating(4.6));
