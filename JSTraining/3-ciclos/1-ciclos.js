...
// Para iniciar tenemos una variable const con un stirng al cual le aplicaremos una logica
// declaramos otra variable adicional vacia la cual vamos a guardar la logica final del codigo, esta variable esta declarada con let ya que su valor va a estar en cambio
// aplicamos un for que esta iniciando en la pocision 0 y me va a recorrer todo el sting de la primera variable
// utilizamos un condicional if, donde le damos como condicion (si la variable que estamos recorriendo en la pocision actual se encuentra con un espacio) si la condicion se cumple,
// pasamos a ejecutar el cuerpo de ese if (donde vamos a guardar en la segunda variable que creamos el nuevo texto reemplazando los espacios por un guion bajo)
// si la condicion no se cumple pues seguimos guardando el string (en este caso las palabras) tal cual como esta.
//por ultimo vamos a imprimir
